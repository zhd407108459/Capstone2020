...
public class SilenceDebuff : BasicDebuff
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.tag.Equals("Player"))
        {
            ...
            GameManager.instance.player.GetComponent<PlayerAction>().StartSilence(effectTime);
    ...
    private void CheckGetAllDebuffTrapAchievement()
    {
        ...
        if (SteamClient.IsValid)
        {
            ...
            if (!ach.State)
            {
                ...
                if (fsilence == 0)
                {
                    SteamUserStats.SetStat("IS_UNLOCK_SILENCE_DEBUFF", 1);
                }
                ...
                fsilence = SteamUserStats.GetStatInt("IS_UNLOCK_SILENCE_DEBUFF");
